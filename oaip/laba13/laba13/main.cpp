...
{
	double A = 0.6180339887499; // Константа A для мультипликативного хеширования
	double hash = size * (key * A - int(key * A));
	return (p + key) % size;
}
...
//Это весь код программы. Дальше я буду кидать отдельные фрагменты кода, которые нужны для конкретного дополнительного задания.